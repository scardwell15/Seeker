...
//        ship.getTravelDrive().forceState(ShipSystemAPI.SystemState.OUT, 1);
    ...
    */
    private ShipAPI findSuitableTarget(CombatEngineAPI engine, FleetSide side) {
        ...
        if (ships.containsKey(ShipAPI.HullSize.CAPITAL_SHIP))
            return engine.getFleetManager(side).getShipFor(ships.get(ShipAPI.HullSize.CAPITAL_SHIP));
        if (ships.containsKey(ShipAPI.HullSize.CRUISER))
            return engine.getFleetManager(side).getShipFor(ships.get(ShipAPI.HullSize.CRUISER));
        if (ships.containsKey(ShipAPI.HullSize.DESTROYER))
            return engine.getFleetManager(side).getShipFor(ships.get(ShipAPI.HullSize.DESTROYER));
        if (ships.containsKey(ShipAPI.HullSize.FRIGATE))
            return engine.getFleetManager(side).getShipFor(ships.get(ShipAPI.HullSize.FRIGATE));
        return null;
    }
